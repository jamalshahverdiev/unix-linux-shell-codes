...
configure_hosts () {
    ...
    IP=$(grep $CURRHOST $HOSTS | awk '{print $1}')
    sed -i "/$CURRHOST/s/^/#/" $HOSTS
    printf '%s\t%s.%s\t%s\n' $IP $CURRHOST $DOMAIN $CURRHOST >> $HOSTS
}

configure_network () {
    echo "Configuring network file" >> $LOG
...
create_pam_system_auth_custom() {
	echo "Creating template system_auth_custom file" >> $LOG
	cat << EOF > ${PAMCONF}
#%PAM-1.0
...
password    requisite     pam_cracklib.so try_first_pass retry=3 minlen=12 lcredit=-1 ucredit=-1 dcredit=-1 ocredit=-1 difok=3
...
EOF
    ...
    echo "Backing up system_auth_custom file" >> $LOG
	cp ${PAMCONF} ${PAMCONF}.$(date +%Y%m%d-%H%M%S)
}

create_pam_password_auth_custom() {
	echo "Creating template password_auth_custom file" >> $LOG
	cat << EOF > ${PAMPWCONF}
#%PAM-1.0
# This file is auto-generated.
# User changes will be destroyed the next time authconfig is run.
...
EOF
    ...
    echo "Backing up password_auth_custom file" >> $LOG
